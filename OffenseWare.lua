...
function Library:CreateWindow(HubName)
    ...
    SubTitle.Font = Enum.Font.Gotham; SubTitle.Text = "VERSION 8"; SubTitle.TextColor3 = Theme.TextDim; SubTitle.TextSize = 11; SubTitle.TextXAlignment = Enum.TextXAlignment.Left
    ...
    function WinFuncs:CreateTab(Name)
        ...
        if FirstTab then
            TabBtn.TextColor3 = Theme.Text; Ind.Visible = true; Page.Visible = true
            FirstTab = false
        end
        ...
        TabBtn.MouseButton1Click:Connect(function()
            for _,v in pairs(TabContainer:GetChildren()) do
                if v:IsA("TextButton") then
                    Tween(v, {TextColor3 = Theme.TextDim})
        ...
        function Elements:CreateToggle(Text, Default, Callback)
            ...
            Tog.MouseButton1Click:Connect(function()
                Toggled = not Toggled
                Tween(Switch, {BackgroundColor3 = Toggled and Theme.Main or Color3.fromRGB(50,50,55)})
        ...
        function Elements:CreateDropdown(Text, Items, Default, Callback)
            ...
            for _, Item in pairs(Items) do
                ...
                ItemBtn.MouseButton1Click:Connect(function()
                    Selected = Item
                    SelLab.Text = Selected -- UPDATE TEXT INSTANTLY
                    pcall(Callback, Item)
                    
                    -- Close Dropdown
                    DropOpen = false
                    Tween(DropFrame, {Size = UDim2.new(1, 0, 0, 34)}, 0.3)
                    Tween(Arrow, {Rotation = 0}, 0.3)
                    Tween(DS, {Color = Theme.Outline})
                end)
            end

            Trig.MouseButton1Click:Connect(function()
                DropOpen = not DropOpen
                local TargetH = DropOpen and (40 + (#Items * 30)) or 34
                Tween(DropFrame, {Size = UDim2.new(1, 0, 0, TargetH)}, 0.3)
                Tween(Arrow, {Rotation = DropOpen and 180 or 0}, 0.3)
                Tween(DS, {Color = DropOpen and Theme.Main or Theme.Outline})
            end)
        end
